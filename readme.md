...
- **轻量信任模型**：插件运行在独立 iframe 中，后端 API 通过命名空间隔离互不干扰；`permissions` 仅作知情明示、不做运行时强制——本地工具信任来自源码可见与自行安装，真正的隔离留给独立 runtime 进程（见 docs/core-direction.md）。
- **易于分发**：主程序可打包为便携目录/压缩包，插件可离线安装。
...
- 设置页提供 🎨 **外观设置面板**：浅色/深色切换 + 13 项 CSS 变量颜色自定义（下拉框选择预设色）。
