...
### 2.3 已处置：共享项容量上限（按实测放宽为默认不限制）

* **原状**：设计文档 §13 第 2 项自标 `[部分]`，`frontend/js/app.js:348` 直接发送
  `max_bytes: 1024 * 1024 * 1024`，而 `settings_schema` 里没有该项 —— 用户既没有
  入口，也会把 1 GiB 当成协议上限。
* **实测它限不住**（这才是放宽的真正理由，不是"懒得做设置项"）：
  `node.py` 的判定是"**每条连接**量一次基线，之后按基线推算"。实测 8 条并发连接
  在"上限 1000 字节"的共享项上各写 400 字节，**全部返回成功**，落盘 3200 字节。
  另有两处天然缺口：它是事后估算（拦不住别的进程往同一目录写），
  也拦不住发起方先探询再分块。
* **处置（2026-09-17）**：`DEFAULT_MAX_BYTES = None`；插件的 `add_share()` 不再带
  `max_bytes` 参数（不传 / 传 `0` / 传 `null` = 不限制），界面不再发送硬编码值；
  `node.py` 的判定逻辑**保留**，显式设置上限时仍然生效（用例覆盖"显式上限被遵守"
  与"显式非法值被拒"）。**不做界面设置项**：给不出可信保证的旋钮比没有旋钮更糟。
* **代价**：被授予 `write` 的人可以写满属主磁盘。设计文档 §6.5 因此改为写明
  "不要给不信任的人 write 权限"是唯一有效的边界。
...
### 2.6.1 已修：改了不兼容的线格式却没递增协议版本号

* **现象（本轮自己造成的，值得留档）**：把 X25519 编码改成 RFC 7748 时忘了递增
  `PROTO_VERSION`，于是新旧节点都自报"版本 1"。`Negotiation.resolve()` 本来会给出
  "协议主版本不一致（本地 X，对端 Y），请升级后再连"这样可读的拒绝，但版本号没变
  就轮不到它 —— 用户看到的是一句 MAC 校验失败。
* **修复**：`PROTO_VERSION = 2`，并在 `NegotiationTest` 里加一条
  "本机 hello 必须报出 `PROTO_VERSION` 本身"的用例，让"改了线格式却没动版本号"
  至少被测试提醒一次；协商用例的 `proto` 改为引用常量而非字面量 1。
* **附带发现**：原用例把 `proto: 1` 写死在三处，因此**版本号与实现可以在测试全绿的
  情况下漂移**。这类"测试复制了被测常量"的写法是同一个坑的温床，已在注释里写明。
...
| 7 | ~~名单自动分发（注册记录承载）~~ **已完成**（`op=roster`，见 §5.7 现状） | — | — |
| 8 | 内容寻址分块传输（§10） | 无 | 大 |
