class MangaLibrary {
    ...
    async init() {
        ...
        const gridOptions = {
            cardRenderer: (manga) => ({
                image: manga.cover_url ? Bridge.originalUrl(manga.cover_url) : '',
                title: manga.title,
    ...
    showChapters() {
        ...
        const chapterGrid = createCardGrid(contentGrid, {
            cardRenderer: (ch) => ({
                image: ch.cover_url ? Bridge.originalUrl(ch.cover_url) : '',
    ...
    async showImages(folderName, chapterPath) {
        ...
        try {
            ...
            const imageGrid = createCardGrid(contentGrid, {
                cardRenderer: (item) => ({
                    image: item.url ? Bridge.originalUrl(item.url) : '',
                    title: '', subtitle: '', badge: ''
                }),
                onClick: (item, index) => {
                    this.reader.open(pages, index);
                }
            });
    ...
    bindSearch() {
        ...
        input.addEventListener('input', Utils.debounce(async (e) => {
            ...
            try {
                ...
                document.getElementById('manga-home').scrollTo({ top: 0 });
            } catch (e) {
