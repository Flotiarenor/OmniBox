"""image-viewer 前端脚本「装载契约」的回归（tests/js/image_viewer_app_split.mjs）。
...
harness 按 `index.html` 的声明顺序在 vm 里装载插件前端的全部脚本并逐条断言，因此它同时是
"把 1500 行的 `image-viewer/frontend/js/app.js` 拆成多个分片"这一步的护栏：拆分前后都应
通过，且**新增分片只要出现在 `index.html` 里就会被自动纳入检查**，不需要改 harness。

检查器本身是共享的（`tests/js/script_load_contract.mjs`），media-player 用的是同一份。

环境无 node 时跳过（前端构建本身依赖 node，CI 上不会真的缺）。

运行：
    python -m unittest tests.test_image_viewer_scripts_js -v
"""
...
HARNESS = PROJECT_ROOT / 'tests' / 'js' / 'image_viewer_app_split.mjs'


@unittest.skipUnless(shutil.which('node'), '未检测到 node，跳过前端脚本装载契约用例')
class ImageViewerScriptLoadContractTest(unittest.TestCase):
