...
`roots[0]` 解析 → 正常打开。若把它加成人生的"额外根"，原图会 404（壳的 `/file` 相对路径
分支只认第一根，见 `.dsh/group-mesh-materialize.md` §2.2）。所以本演示**不保存设置**：
既避免同一目录以"第一根子目录"和"额外根"两种身份重复出现，也不去撞缺陷③。
缺陷③修好之后，"镜像放任何地方 + 加进列表"才是干净解法。
