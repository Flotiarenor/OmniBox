 ...
 * 已通知状态对应的"文档代次"。
 *
 * 插件被重载（错误重试、设置变更改 src，或它本就不保活、每次进入都是新文档）时 iframe
 * 换了新文档，插件前端的可见性认知回到初始值，而宿主的 lastVisible 还停在旧文档上 ——
 * 不同步的话插件会永久停在错误的假设里（例如后台重载的新文档以为自己可见，白跑轮询）。
 * 代次变化即视为"未通知过"。
 */
 ...
 *
 * 必须显式登记：不保活的插件在 iframe 加载完成前就可能被切走，此时它还没有过
